...
class State(object):
    ...
    def login(cls, email, password, on_2fa_callback, user_agent=None):
        ...
        soup = find_input_fields(session.get("https://facebook.com/login.php").text)
        data = dict(
       ...
       # print(data)
        r = session.post("https://facebook.com/login.php?login_attempt=0&lwv=100", data=data)
    ...
    def is_logged_in(self):
     ...
     #   print(self._session.headers)
        ...
        r = self._session.get(url, allow_redirects=False, headers=headers)
        print(r.headers)
     #   print(r.text)
